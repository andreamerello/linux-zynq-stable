...
static void xiic_start_recv(struct xiic_i2c *i2c)
{
	...
	/* Looks like the Hw starts immediately after writing the slave address
	 * into the TX FIFO, and it tries to fetch the second word from the TX
	 * fifo as soon as it's ready. If the SW has not yet wrote it, RX errors
	 * happens.
	 *
	 * Trying to simply disable/enable the peripheral seems to have nasty
	 * side effects.
	 *
	 * Possibly some way to fix this could exist (not using
	 * dynamic mode ?); int the mean time workaround this by trying to
	 * perform the two writes back-to-back.
	 */
